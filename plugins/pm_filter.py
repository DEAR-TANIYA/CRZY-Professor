...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("ബ്രോ, മറ്റുള്ളവർ റിക്വസ്റ്റ്റി ചെയിത മൂവിയിൽ കുത്തി നോക്കാതെ ബ്രോന് വേണ്ടത് റിക്വസ്റ്റ് ചെയ്യുക.👍", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("ബ്രോ, മറ്റുള്ളവർ റിക്വസ്റ്റ്റി ചെയിത മൂവിയിൽ കുത്തി നോക്കാതെ ബ്രോന് വേണ്ടത് റിക്വസ്റ്റ് ചെയ്യുക.👍", show_alert=True)
        return
    ...
    if n_offset == 0:
        btn.append(
            [InlineKeyboardButton("⌫ 𝗕𝗔𝗖𝗞", callback_data=f"next_{req}_{key}_{off_set}"),
             InlineKeyboardButton(f"⎙ 𝗣𝗔𝗚𝗘𝗦 {round(int(offset) / 10) + 1} / {round(total / 10)}",
                                  callback_data="pages")]
        )
    elif off_set is None:
        btn.append(
            [InlineKeyboardButton(f"⎙ 𝗣𝗔𝗚𝗘𝗦 {round(int(offset) / 10) + 1} / {round(total / 10)}", callback_data="pages"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if (st.status == "creator") or (str(userid) in ADMINS):
            ...
            await query.answer("Sona! Aisa kre ke liye aapko Group Owner banna prega ya ek Authorised user hona prega !", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('Sᴇᴛᴛɪɴɢs', callback_data='extra')
        ], [
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⚠️Bᴜᴛᴛᴏɴ⚠️', callback_data='button')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text=f"⎙ 𝗣𝗔𝗚𝗘𝗦 1/{round(int(total_results) / 10)}", callback_data="pages"),
    ...
    if imdb:
        ...
        cap = f"<b>⍞ 𝗧𝗶𝘁𝗹𝗲 : {search}</b>\n<b>❂ 𝗥𝗲𝗾𝘂𝗲𝘀𝘁𝗲𝗱 𝗯𝘆 : {message.from_user.mention}</b>\n<b>⏣ 𝗖𝗵𝗮𝗻𝗻𝗲𝗹 :<a href=https://t.me/+ZiDeeN9yubk5NDhl>ᴄɪɴᴇᴍᴀ ᴄɪᴛʏ</a></b>\n\n<b>𝚄𝙿𝙻𝙾𝙰𝙳𝙴𝙳 𝙱𝚈 : CiNEMA CiTY 🍿GROUP</b>™️"
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        k = await msg.reply("ബ്രോ, മറ്റുള്ളവർ റിക്വസ്റ്റ്റി ചെയിത മൂവിയിൽ കുത്തി നോക്കാതെ ബ്രോന് വേണ്ടത് റിക്വസ്റ്റ് ചെയ്യുക.👍")
